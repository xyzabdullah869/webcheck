...
export default function InstructorEditCoursePage() {
  ...
  const handleSave = async (newStatus?: string) => {
    ...
    else { toast({ title: 'Course updated' }); router.push('/instructor/courses'); }
  };
  ...
  return (
    <PageTransition>
      <div className="space-y-6">
        <Button asChild variant="ghost" size="sm"><Link href="/instructor/courses"><ArrowLeft className="mr-2 h-4 w-4" />My Courses</Link></Button>
        <div>
          <h1 className="font-display text-2xl font-bold sm:text-3xl">Edit Course</h1>
          <p className="mt-1 text-muted-foreground">Update your course details and publication status.</p>
        </div>
        <div className="space-y-6">
