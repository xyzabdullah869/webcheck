...
export default function AdminCreateCoursePage() {
  ...
  const [form, setForm] = React.useState({
    ...
    featured: false, certificate_enabled: true,
  });
  ...
  const handleSubmit = async (e: React.FormEvent, status: 'Draft' | 'Published') => {
    ...
    const { error } = await supabase.from('courses').insert({
      title: form.title, slug, short_description: form.short_description, description: form.description,
      price: form.price, price_pkr: form.price_pkr, original_price: form.original_price || null, level: form.level, duration: form.duration,
      language: form.language, thumbnail: form.thumbnail || null, trailer_url: form.trailer_url || null,
      category_id: form.category_id || null, instructor_id: user.id, status, lessons_count: 0, students_count: 0,
      rating: 0, reviews_count: 0, bestseller: false, is_new: true, featured: form.featured, certificate_enabled: form.certificate_enabled,
      tags: [], what_you_will_learn: [], requirements: [],
    });
    setSaving(false);
    if (error) toast({ title: 'Error', description: error.message, variant: 'destructive' });
    else { toast({ title: `Course ${status === 'Published' ? 'published' : 'saved as draft'}` }); window.location.href = '/admin/courses'; }
  };
    ...
    <PageTransition>
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="mt-1 text-muted-foreground">Fill in the details below to create a new course.</p>
        </div>
        <form className="space-y-6">
          <Card className="space-y-5 p-6 shadow-soft">
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              <div className="space-y-2 sm:col-span-2">
                ...
                <Input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value, slug: generateSlug(e.target.value) })} placeholder="Introduction to Bioinformatics" required />
              </div>
              <div className="space-y-2"><Label>URL Slug</Label><Input value={form.slug} onChange={(e) => setForm({ ...form, slug: e.target.value })} placeholder="introduction-to-bioinformatics" className="font-mono text-sm" /></div>
              ...
              <div className="space-y-2 sm:col-span-2"><Label>Short Description</Label><Input value={form.short_description} onChange={(e) => setForm({ ...form, short_description: e.target.value })} placeholder="A brief one-line description" /></div>
              <div className="space-y-2 sm:col-span-2"><Label>Full Description</Label><Textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={4} placeholder="Detailed course description..." /></div>
            </div>
          </Card>
          <Card className="space-y-5 p-6 shadow-soft">
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              ...
              <div className="space-y-2"><Label>Original Price ($)</Label><Input type="number" step="0.01" min="0" value={form.original_price} onChange={(e) => setForm({ ...form, original_price: parseFloat(e.target.value) || 0 })} placeholder="For showing discount" /></div>
            </div>
          </Card>
          <Card className="space-y-5 p-6 shadow-soft">
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              ...
              <div className="space-y-2"><Label>Trailer Video URL</Label><Input value={form.trailer_url} onChange={(e) => setForm({ ...form, trailer_url: e.target.value })} placeholder="https://youtube.com/..." /></div>
              ...
              <div className="space-y-2"><Label>Duration</Label><Input value={form.duration} onChange={(e) => setForm({ ...form, duration: e.target.value })} placeholder="10 hours" /></div>
              <div className="space-y-2"><Label>Language</Label><Input value={form.language} onChange={(e) => setForm({ ...form, language: e.target.value })} placeholder="English" /></div>
            </div>
          ...
          <div className="flex gap-2">
            <Button type="button" onClick={(e) => handleSubmit(e as unknown as React.FormEvent, 'Published')} disabled={saving}>
              {saving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Eye className="mr-2 h-4 w-4" />}Publish Course
            </Button>
            <Button type="button" variant="outline" onClick={(e) => handleSubmit(e as unknown as React.FormEvent, 'Draft')} disabled={saving}>
              {saving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}Save as Draft
            </Button>
          </div>
        </form>
      </div>
