...
import { FileText, Plus, Trash2, CreditCard as Edit3, Loader as Loader2, X, Calendar, Award } from 'lucide-react';
...
export default function AdminAssignmentsPage() {
  ...
  const [deleteTarget, setDeleteTarget] = React.useState<AssignmentWithCourse | null>(null);
  ...
  React.useEffect(() => {
    (async () => {
      ...
      setCourses((courseData ?? []).map((c: Record<string, unknown>) => ({ id: c.id as string, title: c.title as string })));
      const { data: moduleData } = await supabase
        .from('modules')
        .select('id, title, course_id')
        .order('order_index', { ascending: true });
      setModules((moduleData ?? []).map((m: Record<string, unknown>) => ({ id: m.id as string, title: m.title as string, course_id: m.course_id as string })));
    })();
    loadAssignments();
  }, [loadAssignments]);

  const resetForm = () => {
    setForm({ courseId: '', moduleId: null, title: '', description: '', dueDate: '', maxScore: 100, allowedFileTypes: 'PDF,DOCX,ZIP' });
    setEditing(null);
  };
  ...
  const openEdit = (a: AssignmentWithCourse) => {
    setForm({
      courseId: a.courseId,
      moduleId: a.moduleId,
      title: a.title,
      ...
      maxScore: a.maxScore,
      allowedFileTypes: a.allowedFileTypes.join(','),
    });
    setEditing(a);
    setShowForm(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      allowedFileTypes: form.allowedFileTypes.split(',').map((s) => s.trim().toUpperCase()).filter(Boolean),
    };
  ...
  return (
    <PageTransition>
      <div className="space-y-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-muted-foreground">Create assignments, review submissions, and provide feedback.</p>
          </div>
        ...
        {loading ? (
                ...
                <motion.div key={a.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}>
                  <Card className="p-5 shadow-soft transition-all hover:shadow-card">
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                      ...
                      {a.dueDate && (
                        <span className={`flex items-center gap-1 ${isOverdue ? 'text-rose-600' : ''}`}>
          ...
          <Card className="p-6 shadow-soft">
            <EmptyState
              ...
              description="Create assignments for your courses and track student submissions."
              ...
              >
                <Card className="max-h-[90vh] overflow-y-auto p-6 shadow-float">
                  <div className="flex items-center justify-between">
                    <h3 className="font-display text-lg font-bold">{editing ? 'Edit Assignment' : 'New Assignment'}</h3>
                  ...
                  <form onSubmit={handleSubmit} className="mt-4 space-y-4">
                    <div className="space-y-2">
                      ...
                      <select
                        value={form.courseId}
                        onChange={(e) => setForm({ ...form, courseId: e.target.value, moduleId: null })}
                    ...
                    <div className="space-y-2">
                      ...
                      <select
                        value={form.moduleId ?? ''}
                        onChange={(e) => setForm({ ...form, moduleId: e.target.value || null })}
                        className="h-9 w-full rounded-md border bg-background px-3 text-sm"
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        placeholder="DNA Sequence Analysis Project"
                        required
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        placeholder="Analyze a given DNA sequence and submit a report"
                      />
                    </div>
      ...
      <ConfirmDialog
        ...
        description={`Are you sure you want to delete "${deleteTarget?.title}"? All student submissions will also be deleted. This action cannot be undone.`}
        confirmLabel="Delete"
        variant="destructive"
