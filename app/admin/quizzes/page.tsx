...
export default function AdminQuizzesPage() {
  ...
  const [deleteTarget, setDeleteTarget] = React.useState<QuizWithCourse | null>(null);
  ...
  const resetForm = () => {
    setForm({ courseId: '', moduleId: null, title: '', description: '', passingScore: 70, timeLimit: 600 });
    setEditing(null);
  };
  ...
  const openEdit = (quiz: QuizWithCourse) => {
    setForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      toast({ title: editing ? 'Quiz updated' : 'Quiz created' });
      setShowForm(false);
      resetForm();
      loadQuizzes();
    } else {
    ...
    <PageTransition>
      <div className="space-y-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="font-display text-2xl font-bold sm:text-3xl">Quizzes</h1>
            <p className="mt-1 text-muted-foreground">Create and manage quizzes with MCQ and True/False questions.</p>
          </div>
        ...
        {loading ? (
                ...
                <Card className="p-5 shadow-soft transition-all hover:shadow-card">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex gap-1">
                      ...
                      <Button size="sm" variant="ghost" onClick={() => openEdit(quiz)}>
                        <Edit3 className="h-3.5 w-3.5" />
                      </Button>
                      <Button size="sm" variant="ghost" className="text-rose-600" onClick={() => setDeleteTarget(quiz)}>
                  ...
                  <Button asChild size="sm" variant="outline" className="mt-3 w-full">
                    <Link href={`/admin/quizzes/${quiz.id}`}>
                      Manage Questions <ArrowRight className="ml-1 h-3 w-3" />
                    </Link>
                  </Button>
                </Card>
              </motion.div>
          ...
          <Card className="p-6 shadow-soft">
            <EmptyState
              ...
              description="Create quizzes with MCQ and True/False questions for your courses."
              action={{ label: 'Create Quiz', onClick: openCreate }}
            />
          </Card>
        )}
              ...
              >
                <Card className="max-h-[90vh] overflow-y-auto p-6 shadow-float">
                  <div className="flex items-center justify-between">
                    <h3 className="font-display text-lg font-bold">{editing ? 'Edit Quiz' : 'New Quiz'}</h3>
                  ...
                  <form onSubmit={handleSubmit} className="mt-4 space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        placeholder="Introduction to DNA Quiz"
                        required
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        placeholder="Test your knowledge on DNA basics"
                      />
                    </div>
      ...
      <ConfirmDialog
        ...
        title="Delete Quiz"
        description={`Are you sure you want to delete "${deleteTarget?.title}"? All questions and results will also be deleted. This action cannot be undone.`}
        confirmLabel="Delete"
        variant="destructive"
