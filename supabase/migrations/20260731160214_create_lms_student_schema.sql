...
## Overview
Creates all student-interaction tables for the LMS.

## New Tables
1. **enrollments** — tracks which student enrolled in which course
2. **lesson_progress** — tracks per-lesson completion and video position
3. **quizzes** — quiz definitions per course/module
4. **quiz_questions** — individual quiz questions with options
5. **quiz_results** — student quiz attempt results
6. **assignments** — assignment definitions per course
7. **assignment_submissions** — student submissions for assignments
8. **certificates** — issued certificates with verification IDs
9. **reviews** — course reviews by students
10. **bookmarks** — bookmarked courses/lessons by students
11. **notifications** — user notification center

## Security
- All student tables are owner-scoped (auth.uid() = user_id) for CRUD
- Public can read reviews for published courses
- Quiz questions/answers are read-restricted during active attempts
- Certificate verification is public read (for verification page)
- Admin/instructors have elevated access via has_role()
*/
...
-- Public can verify by certificate_id (for verification page)
-- We use a separate policy with anon access limited to certificate lookup
...
-- Only instructors/admins can issue certificates (insert)
