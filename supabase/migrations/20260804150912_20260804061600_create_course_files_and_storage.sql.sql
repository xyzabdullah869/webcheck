...
DROP POLICY IF EXISTS "select_course_files" ON course_files;
  ...
  TO authenticated USING (
    ...
    OR EXISTS (
      ...
      WHERE c.id = course_files.course_id AND (c.instructor_id = auth.uid() OR EXISTS (
        SELECT 1 FROM profiles p WHERE p.id = auth.uid() AND p.role IN ('admin', 'owner')
      ))
    )
  );

DROP POLICY IF EXISTS "insert_course_files" ON course_files;
...
DROP POLICY IF EXISTS "update_course_files" ON course_files;
...
DROP POLICY IF EXISTS "delete_course_files" ON course_files;
...
VALUES (
  'course-files', 'course-files', false, 524288000,
  ARRAY[
...
CREATE POLICY "course_files_upload" ON storage.objects FOR INSERT
  TO authenticated WITH CHECK (
    ...
    AND EXISTS (
      SELECT 1 FROM profiles p WHERE p.id = auth.uid() AND p.role IN ('admin', 'owner', 'instructor')
    )
  );
...
CREATE POLICY "course_files_read" ON storage.objects FOR SELECT
  TO authenticated USING (bucket_id = 'course-files');