...
-- ========================
-- ROLE HELPER FUNCTION (updated to include 'owner')
-- ========================
...
AS $$
  SELECT EXISTS (
    ...
    WHERE id = auth.uid()
    AND (
      (required_role = 'student' AND role IN ('student', 'instructor', 'admin', 'owner'))
      OR (required_role = 'instructor' AND role IN ('instructor', 'admin', 'owner'))
