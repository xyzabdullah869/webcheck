...
1. New Tables
- `payment_settings` — global payment configuration (currency, tax rate, commission rates)
- `coupons` — discount codes with percentage/fixed discounts, expiry, usage limits
- `orders` — student purchase orders with payment status, gateway, coupon, breakdown
- `order_items` — individual course line items within an order
- `instructor_earnings` — per-course earnings records for instructors
- `withdrawal_requests` — instructor withdrawal requests with admin approval flow

2. Modified Tables
- No existing tables are modified (no column drops, renames, or type changes)

3. Security
- RLS enabled on all new tables
- payment_settings: SELECT public (anon+authenticated), INSERT/UPDATE admin only
- coupons: SELECT public, INSERT/UPDATE/DELETE admin only
- orders: SELECT/INSERT owner-scoped (student), UPDATE admin only (for status changes)
- order_items: SELECT owner-scoped (student via order join), INSERT with order
- instructor_earnings: SELECT instructor-scoped, INSERT system/admin
- withdrawal_requests: SELECT instructor-scoped, INSERT instructor, UPDATE admin

4. Important Notes
- Uses existing `courses`, `profiles`, `enrollments` tables
- Commission settings stored in payment_settings for simplicity
- Orders track full price breakdown (subtotal, discount, tax, total)
- Enrollments auto-created on successful payment (handled in application layer)
- All monetary columns use numeric(12,2) for precision
*/

-- ============================================================
-- PAYMENT SETTINGS (single-row config table)
-- ============================================================
